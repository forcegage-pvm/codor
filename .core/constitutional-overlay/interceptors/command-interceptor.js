...
class ConstitutionalInterceptor {
  ...
  async loadConfig() {
    try {
      ...
      console.log(
        "⚠️  Constitutional config not found, generating from constitution..."
      );
  ...
  async generateConfigFromConstitution() {
    try {
      const constitutionContent = await fs.readFile(
      ...
      console.error(
        "❌ Failed to generate constitutional config:",
        error.message
      );
      throw error;
    }
  ...
  parseConstitutionMandates(content) {
    ...
    const mandateMatches = content.match(
      /### MANDATE (\d+): ([^\\n]+)[\\s\\S]*?(?=### MANDATE|\n---|\n##|$)/g
    );
      ...
      mandateMatches.forEach((match) => {
        ...
        mandates[`MANDATE_${number}`] = {
          ...
          antiCircumnavigation:
            match.includes("ANTI-CIRCUMNAVIGATION") ||
            match.includes("fraud detection"),
          threeGateValidation:
            match.includes("three-gate") || match.includes("Three-Gate"),
          evidenceFirst:
            match.includes("evidence") || match.includes("Evidence"),
          specToCompliance:
            match.includes("spec-to-compliance") ||
            match.includes("Spec-to-Compliance"),
        };
      });
    }
  ...
  async interceptCommand(command, args) {
    ...
    switch (command) {
      ...
      default:
        console.log(
          `⚠️  No constitutional enhancement for /${command}, executing vanilla command`
        );
  ...
  async enhanceTasks(args) {
    ...
    try {
      ...
      if (!tasksPath) {
        console.log(
          "⚠️  No tasks.md file found, vanilla command may have failed"
        );
      ...
      const enhancedTasks = await this.applyConstitutionalEnhancements(
        tasksPath
      );
...
if (require.main === module) {
  ...
  if (!command) {
    ...
    console.log(
      "  tasks     - Enhance task generation with constitutional compliance"
    );
    console.log(
      "  plan      - Add constitutional checks to implementation plans"
    );
    console.log(
      "  implement - Add validation gates to implementation workflow"
    );
    console.log(
      "  specify   - Enhance specifications with constitutional requirements"
    );
    process.exit(1);
  }
