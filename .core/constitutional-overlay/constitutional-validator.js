...
const { ConstitutionalAutoDiscovery } = require('./constitutional-auto-discovery');
const fs = require('fs');
...
class ConstitutionalComplianceValidator {
    ...
    async testConstitutionalAutoDiscovery() {
        ...
        try {
            ...
            const constitutionalSystem = new ConstitutionalAutoDiscovery(this.workspacePath);
    ...
    async testEvidenceGeneration() {
        ...
        try {
            ...
            const check1 = {
                ...
                actual: typeof global.GENERATE_CONSTITUTIONAL_EVIDENCE === 'function' ? 'AVAILABLE' : 'NOT_AVAILABLE',
                status: typeof global.GENERATE_CONSTITUTIONAL_EVIDENCE === 'function' ? 'PASS' : 'FAIL'
            };
            test.checks.push(check1);
    ...
    async testAuditTrail() {
        ...
        try {
            ...
            const check1 = {
                ...
                actual: Array.isArray(global.CONSTITUTIONAL_AUDIT_TRAIL) ? 'AVAILABLE' : 'NOT_AVAILABLE',
                status: Array.isArray(global.CONSTITUTIONAL_AUDIT_TRAIL) ? 'PASS' : 'FAIL'
            };
            test.checks.push(check1);
            ...
            const check2 = {
                ...
                actual: typeof global.LOG_CONSTITUTIONAL_ACTION === 'function' ? 'AVAILABLE' : 'NOT_AVAILABLE',
                status: typeof global.LOG_CONSTITUTIONAL_ACTION === 'function' ? 'PASS' : 'FAIL'
            };
            test.checks.push(check2);
    ...
    async testEnvironmentalMarkers() {
        ...
        try {
            ...
            const check2 = {
                ...
                status: process.env.CONSTITUTIONAL_COMPLIANCE === 'ACTIVE' ? 'PASS' : 'FAIL'
            };
            test.checks.push(check2);
            console.log(`  ✓ CONSTITUTIONAL_COMPLIANCE: ${check2.status} (${check2.actual})`);
    ...
    async testAntiCircumnavigation() {
        ...
        try {
            ...
            const check2 = {
                ...
                actual: typeof evidenceFunction === 'function' ? 'PROTECTED' : 'VULNERABLE',
            ...
            test.status = test.checks.filter(c => c.status !== 'WARNING').every(c => c.status === 'PASS') ? 'PASS' : 'FAIL';
    ...
    generateValidationReport() {
        const reportPath = path.join(this.workspacePath, '.codor', 'evidence', 'constitutional-validation-report.md');
...
**Key Findings**:
- Constitutional Auto-Discovery: ${this.getTestStatus('Constitutional Auto-Discovery')}
...
- Environmental Markers: ${this.getTestStatus('Environmental Constitutional Markers')}
...
${this.testResults.filter(t => t.status !== 'PASS').map(t => `- ${t.name}: ${t.status}`).join('\n')}
...
`;
    ...
    getValidationSummary() {
        const passedTests = this.testResults.filter(t => t.status === 'PASS').length;
        const totalTests = this.testResults.length;
        const overallStatus = passedTests === totalTests ? 'CONSTITUTIONAL_COMPLIANCE_ACTIVE' : 'CONSTITUTIONAL_COMPLIANCE_INCOMPLETE';
