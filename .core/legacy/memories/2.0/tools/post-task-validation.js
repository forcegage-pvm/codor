...
class PostTaskValidator {
  ...
  async checkMCPEvidence() {
    ...
    const requiredEvidence = [
      ...
      "mcp-screenshots", // Raw JSON responses from MCP take_screenshot
    ];
    ...
    if (missingEvidence.length > 0) {
      this.errors.push(
        `❌ MCP Missing evidence: ${missingEvidence.join(", ")}\n` +
          "📋 REQUIRED ACTION: Create proper evidence structure\n" +
          "🔧 EVIDENCE REQUIREMENTS:\n" +
          "   • mcp-interaction.log: Log of all MCP browser commands and responses\n" +
          "   • functional-test-results.json: Results from actual functional testing\n" +
          "   • mcp-screenshots/: Directory with BOTH take_snapshot.json AND take_screenshot.json\n" +
          "   📝 NOTE: VS Code MCP returns OCR text, not binary images - JSON responses are the valid evidence\n" +
          ...
          "   4. Save raw JSON responses as: take_snapshot.json and take_screenshot.json in mcp-screenshots/\n" +
          "   5. Document what functionality the OCR text extraction proves\n" +
          "   ⚠️  CRITICAL: BOTH MCP commands required with standardized filenames\n" +
          "⚠️  WARNING: Do not create empty directories or fake files - fraud detection is active"
      );
    } else {
  ...
  async validateMCPScreenshotResponses(mcpResponses, mcpScreenshotsDir) {
    for (const responseFile of mcpResponses) {
      ...
      try {
        const content = JSON.parse(fs.readFileSync(responsePath, "utf8"));
        ...
        if (!content.command || content.command !== expectedCommand) {
          this.errors.push(
            `🚨 FRAUD DETECTED: Invalid MCP response in ${responseFile}\n` +
              `📋 REQUIRED: Response must contain 'command: ${expectedCommand}'\n` +
  ...
  async checkFunctionalBehavior() {
    ...
    if (!fs.existsSync(functionalTestPath)) {
      this.errors.push(
        "❌ MCP Functional test results not found\n" +
          "📋 REQUIRED ACTION: Create functional test results with actual evidence\n" +
          "🔧 PROPER METHOD:\n" +
          "   1. Start dev server and verify it's running\n" +
          "   2. Use MCP browser tools to test actual functionality\n" +
          "   3. Document real interactions, responses, and behaviors\n" +
          "   4. Create functional-test-results.json with 'passed: true' ONLY if tests actually pass\n" +
          "   5. Include evidence/T004/mcp-screenshots/ with raw JSON responses\n" +
          "⚠️  WARNING: Do not manually edit 'passed' field to bypass validation - fraud detection is active"
      );
      return;
    }
    ...
    try {
      ...
      await this.detectFunctionalTestFraud(testResults, functionalTestPath);
      ...
      if (!testResults.passed) {
        this.errors.push(
          "❌ MCP Functional tests failed\n" +
            ...
            "⚠️  WARNING: Only set 'passed: true' when functionality actually works"
        );
      } else {
