...
class PostTaskValidator {
  ...
  async checkMCPEvidence() {
    ...
    const testResultsPath = path.join(this.evidenceDir, "mcp-test-results.json");
    ...
    if (fs.existsSync(testResultsPath) && evidenceRequirements.every(item => 
  ...
  async validateTestResultsFile(testResultsPath) {
    ...
    try {
      ...
      if (testResults.type === "take_screenshot" || testResults.type === "take_snapshot") {
        this.errors.push(
      ...
      if (testResults.functionalTests) {
        const testCategories = ["apiEndpoint", "responseValidation", "businessLogic", "dataIntegrity"];
        const testsPassed = testCategories.every(category => 
  ...
  async validateMCPScreenshotResponses(mcpResponses, mcpScreenshotsDir) {
    for (const responseFile of mcpResponses) {
      ...
      try {
        ...
        console.log(`🔍 DEBUG: Reading ${responseFile}, first 100 chars:`, fileContent.substring(0, 100));
        ...
        if ((!content.command && !content.type) || 
  ...
  async checkFunctionalBehavior() {
    ...
    try {
      ...
      if (testResults.testResultsSummary && testResults.testResultsSummary.overallStatus === "PASS") {
        ...
        const testCategories = ["apiEndpoint", "responseValidation", "businessLogic", "dataIntegrity"];
        const failedTests = testCategories.filter(category => 
          !testResults.functionalTests[category] || 
          testResults.functionalTests[category].status !== "PASS"
        );
        ...
        if (failedTests.length > 0) {
          ...
          console.log("✓ MCP Functional behavior verified (all test categories passed)");
        }
      } else {
        this.warnings.push("Functional test structure not recognized - consider updating test results format");
      }
    } catch (error) {
  ...
  async runConstitutionalChecker() {
    ...
    console.log("✓ Constitutional validation integrated with evidence validation");
